...
for kkk=1:35
    a=k
    aa=kkk
fileID=fopen(['local\',num2str(k),'_',num2str(kkk),'_startpoint_mean_r=0.01_std_r=0.001_step=1_DM=50_0.3_0.3_0.3_per=0.002_molecules=50000_DELTA=0.001_tau=0.1_D=1_2.5_cycle=130.txt']);
...
s0=sqrt(2)/2+sqrt(-1)*sqrt(2)/2;
SNR=20;
signal_noise1=zeros(100,length(thegma1),length(u));
for l=1:100

for t=1:length(thegma1)
for j=1:length(u)
signal_noise1(l,t,j)=norm(real(s0*signal(t,j))+normrnd(0,1/SNR)+sqrt(-1)*(imag(s0*signal(t,j))+normrnd(0,1/SNR)));
end
end
end
signal_noise=zeros(length(thegma1),length(u));
for t=1:length(thegma1)
for j=1:length(u)
    signal_noise(t,j)=mean(signal_noise1(:,t,j));
end
end
...
for j=1:length(u)
    ...
    DTIdata(j).VoxelData = abs(signal_noise(j));
